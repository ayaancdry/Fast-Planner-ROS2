...
void odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  ...
  if (tf45)
  {
    ...
    geometry_msgs::msg::TransformStamped laser_tf = transform45;
    laser_tf.child_frame_id = string("/laser");
    geometry_msgs::msg::TransformStamped vision_tf = transform45;
    vision_tf.child_frame_id = string("/vision");
    geometry_msgs::msg::TransformStamped height_tf = transform90;
    height_tf.child_frame_id = string("/height");

    broadcaster->sendTransform(transform);
    broadcaster->sendTransform(laser_tf);
    broadcaster->sendTransform(vision_tf);
    broadcaster->sendTransform(height_tf);
  }
}
...
int main(int argc, char** argv)
{
  ...
  node_ = std::make_shared<rclcpp::Node>("odom_visualization");

  mesh_resource = node_->declare_parameter("mesh_resource", std::string("package://odom_visualization/meshes/hummingbird.mesh"));
  color_r = node_->declare_parameter("color.r", 1.0);
  color_g = node_->declare_parameter("color.g", 0.0);
  color_b = node_->declare_parameter("color.b", 0.0);
  color_a = node_->declare_parameter("color.a", 1.0);
  origin = node_->declare_parameter("origin", false);
  scale = node_->declare_parameter("robot_scale", 2.0);
  _frame_id = node_->declare_parameter("frame_id", string("world"));

  cross_config = node_->declare_parameter("cross_config", false);
  tf45 = node_->declare_parameter("tf45", false);
  cov_scale = node_->declare_parameter("covariance_scale", 100.0);
  cov_pos = node_->declare_parameter("covariance_position", false);
  cov_vel = node_->declare_parameter("covariance_velocity", false);
  cov_color = node_->declare_parameter("covariance_color", false);
  ...
  auto latched = rclcpp::QoS(100).transient_local();
  posePub   = node_->create_publisher<geometry_msgs::msg::PoseStamped>("pose", latched);
  pathPub   = node_->create_publisher<nav_msgs::msg::Path>("path", latched);
  velPub    = node_->create_publisher<visualization_msgs::msg::Marker>("velocity", latched);
  covPub    = node_->create_publisher<visualization_msgs::msg::Marker>("covariance", latched);
  covVelPub = node_->create_publisher<visualization_msgs::msg::Marker>("covariance_velocity", latched);
  trajPub   = node_->create_publisher<visualization_msgs::msg::Marker>("trajectory", latched);
  sensorPub = node_->create_publisher<visualization_msgs::msg::Marker>("sensor", latched);
  meshPub   = node_->create_publisher<visualization_msgs::msg::Marker>("robot", latched);
  heightPub = node_->create_publisher<sensor_msgs::msg::Range>("height", latched);
  tf2_ros::TransformBroadcaster b(node_);
  broadcaster = &b;
