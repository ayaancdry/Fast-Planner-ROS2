...
void load_seg( int segid, const rclcpp::Time& time_base) {
    ...
    if (!this->has_parameter(seg_str + "yaw")) this->declare_parameter<double>(seg_str + "yaw", 0.0);
    this->get_parameter(seg_str + "yaw", yaw);
    assert((yaw > -3.1499999) && (yaw < 3.14999999));
    if (!this->has_parameter(seg_str + "time_from_start")) this->declare_parameter<double>(seg_str + "time_from_start", 0.0);
    ...
    if (!this->has_parameter(seg_str + "x")) this->declare_parameter<std::vector<double>>(seg_str + "x", std::vector<double>());
    if (!this->has_parameter(seg_str + "y")) this->declare_parameter<std::vector<double>>(seg_str + "y", std::vector<double>());
    if (!this->has_parameter(seg_str + "z")) this->declare_parameter<std::vector<double>>(seg_str + "z", std::vector<double>());
...
void load_waypoints(const rclcpp::Time& time_base) {
    ...
    if (!this->has_parameter("segment_cnt")) this->declare_parameter<int>("segment_cnt", 0);
