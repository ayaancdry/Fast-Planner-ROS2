...
void FastPlannerManager::initPlanModules(std::shared_ptr<FastPlanner> nh) {
 ...
 // Decalred paramters from Bspilne optimser to avoid again decalration of same paramters
  ...
  nh->declare_parameter<float>("optimization/visib_min", 0);
  nh->declare_parameter<float>("optimization/dlmin", 0);
  nh->declare_parameter<float>("optimization/wnl", 0);
  ...
  pos_cmd_flag_pub = nh->create_publisher<std_msgs::msg::Bool>("/position_cmd_flag", 50);
...
bool FastPlannerManager::kinodynamicReplan( Eigen::Vector3d start_pt, Eigen::Vector3d start_vel, Eigen::Vector3d start_acc, Eigen::Vector3d end_pt, Eigen::Vector3d end_vel) {
  ...
  if (status == KinodynamicAstar::NO_PATH) {
    ...
    message.data = false; 
    pos_cmd_flag_pub->publish(message);
