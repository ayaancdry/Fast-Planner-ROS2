...
void TopoReplanFSM::init(std::shared_ptr<FastPlanner> nh) {
  current_wp_  = 0;
  ...
  waypoint_sub_ = nh->create_subscription<nav_msgs::msg::Path>(
      "/waypoint_generator/waypoints", 1, std::bind(&TopoReplanFSM::waypointCallback, this, std::placeholders::_1));
...
void TopoReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, std::string pos_call) {
  ...
  std::cout << "[" + pos_call + "]: from " + state_str[pre_s] + " to " + state_str[static_cast<int>(new_state)] << std::endl;
}

void TopoReplanFSM::printFSMExecState() {
  static const vector<std::string> state_str = {"INIT", "WAIT_TARGET", "GEN_NEW_TRAJ", "REPLAN_TRAJ", "EXEC_TRAJ", "REPLAN_NEW"};
  std::cout << "state: " + state_str[static_cast<int>(exec_state_)] << std::endl;
}
  ...
  switch (exec_state_) {
    ...
    case REPLAN_TRAJ: {
      ...
      if (success) {
        ...
        std::cout << "Replan fail, retrying..." << std::endl;
      }
...
void TopoReplanFSM::checkCollisionCallback() {
  ...
  if (exec_state_ == EXEC_TRAJ || exec_state_ == REPLAN_TRAJ) {
    ...
    if (!safe) {
      if (dist > 0.5) {
        std::cout << "current traj " << dist << " m to collision" << std::endl;
        collide_ = true;
        changeFSMExecState(REPLAN_TRAJ, "SAFETY");
      } else {
        std::cout << "current traj " << dist << " m to collision, emergency stop!" << std::endl;
